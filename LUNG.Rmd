...
```{r}
library(readr)
cancer <- read_csv("/Users/batul/Documents/MVA/LUNGCANCER3.csv")
str(cancer)
attach(cancer)
...
library(psych)
library(readr)
data <- read_csv("/Users/batul/Documents/MVA/LUNGCANCER3.csv")
...
lung <- read.csv("/Users/batul/Documents/MVA/LUNGCANCER3.csv")
lung
...
library(car) # for logistic regression
library(readr)
library(MVA)
library(HSAUR2)
library(SciViews)
...
# Load the EFA dataset
efa_data <- read.csv("/Users/batul/Documents/MVA/LUNGCANCER3.csv")

# Run EFA to identify factors
efa_results <- fa(efa_data, nfactors = 3, rotate = "varimax")

# View the factor loadings
print(efa_results$loadings)

# Create a new dataset with the factor scores
factor_scores <- as.data.frame(efa_results$scores)

# Add a column for the outcome variable
factor_scores$outcome <- efa_data$LUNG_CANCER

# Split the dataset into training and test sets
train_index <- sample(1:nrow(factor_scores), nrow(factor_scores) * 0.8)
...
predicted_outcomes <- predict(logit_model, newdata = test_data, type = "response")

predicted_labels <- ifelse(predicted_outcomes > 0.5, 1, 0)
...
roc_curve <- roc(test_data$outcome, predicted_outcomes)
auc_lc <- auc(roc_curve)
auc_lc
ggroc(roc_curve, color = "blue", legacy.axes = TRUE) +
